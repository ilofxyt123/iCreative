(function() {
  ...
  about.prototype = {
    ...
    addEvent: function() {
      ...
      _iCreative.addEvent(
        "#main",
        _iCreative.Event.animationStart,
        _self.PageAStart.bind(_self)
      ); //监听下一页动画结束，结束隐藏本页
      _iCreative.addEvent(
        "#main",
        _iCreative.Event.animationEnd,
        _self.PageAEnd.bind(_self)
      ); //监听下一页动画结束，结束隐藏本页
    },
    ...
    pleave3: function() {
      if (this.wheel.dir == "up") {
        this.ele.$p3.find(".scene-img img").addClass("leave_to_Right100");
        this.ele.$p3.find(".page-bar").addClass("leave_to_Left100");
        this.ele.$p3.find(".htitle,.stitle,p").addClass("leave_to_down");
      }
    },
    pleave4: function() {
      $(".page4").addClass("none");
    ...
    PageAEnd: function(e) {
      ...
      if (_self.wheel.dir == "up") {
        switch (_self.wheel.now) {
          ...
          case 2:
            if (EndDom == _self.ele.$p2.find(".page-bar")[0]) {
              //绿条最后一个消失，已经准备好翻页
            ...
            if (EndDom == _self.ele.$p2[0]) {
              //第二页翻页完成，已经准备好第一页动画
              _self.ele.$p2
                .find(".htitle,.stitle,p,.scene-img img,.page-bar")
                .addClass("opacity")
                .removeClass(
                  "leave_to_down leave_to_Left100 leave_to_Right100"
                );
        ...
        switch (_self.wheel.now) {
          case 1: //page1去page2
            ...
            if (EndDom == _self.ele.$p2[0]) {
              ...
              _self.ele.$p2
                .find(".htitle,.stitle,p")
                .addClass("move_from_down"); //文字从下往上出现
              _self.ele.$p2
                .find(".scene-img img")
                .addClass("move_from_Left100");
              _self.ele.$p2.find(".page-bar").addClass("move_from_Right100");
              return;
            }
            if (EndDom == _self.ele.$p2.find(".htitle")[0]) {
              //第二页全部结束
              ...
              setTimeout(function() {
                _self.ele.$p2
                  .find(".htitle,.stitle,p,.scene-img img,.page-bar")
                  .removeClass(
                    "move_from_Left100 move_from_down move_from_Right100 opacity"
                  );
                _self.isAnimate = false; //动画结束
                _self.wheel.now = _self.wheel.next; //更新当前位置
              }, 600);
          ...
          case 2: //page2去page3
            if (EndDom == _self.ele.$p2_text.find(".htitle")[0]) {
              //page2文字消失
              _self.ele.$p3.find(".page-bar").addClass("move_from_Left100"); //page3绿条从左边出现
              _self.ele.$p3
                .find(".scene-img img")
                .addClass("move_from_Right100"); //page3图片从右边出现
              _self.ele.$p3
                .find(".htitle,.stitle,p")
                .addClass("move_from_down"); //page3文字出现
              return;
            }
            if (EndDom == _self.ele.$p2_text[0]) {
              //page2离开
              _self.ele.$p2
                .removeClass("active bg-color-white")
                .addClass("none"); //page2
              _self.ele.$p2_text
                .removeClass("go_to_Up100")
                .find(".htitle")
                .addClass("opacity")
                .removeClass("fadeOut");
              _self.ele.$p2_text
                .find(".stitle,.pragraph p")
                .removeClass("fadeOut")
                .addClass("opacity");
              _self.ele.$p2
                .find(".scene-img img")
                .removeClass("leave_to_Left100");
            ...
            if (EndDom == _self.ele.$p3.find(".htitle")[0]) {
              ...
              _self.ele.$p3_text
                .find(".htitle,.pragraph p")
                .removeClass("opacity leave_to_down");
              _self.ele.$p3_text
                .find(".page-bar,.scene-img img")
                .removeClass("opacity leave_to_Left100 move_from_Right100");
              _self.isAnimate = false; //动画结束
              _self.wheel.now = _self.wheel.next;
            }
            break;
